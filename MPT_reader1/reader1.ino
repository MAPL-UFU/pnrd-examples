  ...
  if(readError1 == ReadError::NO_ERROR){
    FireError fireError1;
      ...
      if(tagId1 != pnrd1.getTagId()){
        ...
        FireError fireError1 = pnrd1.fire(0);
        ...
        switch(fireError1){
            case FireError::NO_ERROR :
              ...
              if(pnrd1.saveData() == WriteError::NO_ERROR){
                serial_com(tagId1,readerId,1,"NO_ERROR");
                return;
              }else{
                serial_com(tagId1,readerId,1,"ERROR_TAG_UPDATE");
                return;
              }
              return;

            case FireError::PRODUCE_EXCEPTION :
                serial_com(tagId1,readerId,1,"PRODUCE_EXCEPTION");
                break;

            case FireError::CONDITIONS_ARE_NOT_APPLIED :
                serial_com(tagId1,readerId,1,"CONDITIONS_ARE_NOT_APPLIED");
            break;

            case FireError::NOT_KNOWN:
                serial_com(tagId1,readerId,1,"NOT_KNOWN");
            break;             
        }
      }
  }
  if(readError2 == ReadError::NO_ERROR){
    FireError fireError2;
      ...
      if(tagId2 != pnrd2.getTagId()){
        ...
        tagReadyToContinue = false;
        //Realização do disparo da transição T0
        FireError fireError2 = pnrd2.fire(1);
        switch(fireError2){
            case FireError::NO_ERROR :
              
              //Atualiza a tag
              if(pnrd2.saveData() == WriteError::NO_ERROR){
                serial_com(tagId2,readerId,2,"NO_ERROR");
                return;
              }else{
                serial_com(tagId2,readerId,2,"ERROR_TAG_UPDATE");
                return;
              }
              return;
            case FireError::PRODUCE_EXCEPTION :
                serial_com(tagId2,readerId,2,"PRODUCE_EXCEPTION");
                break;

            case FireError::CONDITIONS_ARE_NOT_APPLIED :
                serial_com(tagId2,readerId,2,"CONDITIONS_ARE_NOT_APPLIED");
            break;
            case FireError::NOT_KNOWN:
                serial_com(tagId2,readerId,2,"NOT_KNOWN");
            break;         
        }
      }
  } 
  if(readError3 == ReadError::NO_ERROR){
    ...
    //verifica se é uma nova tag
      if(tagId3 != pnrd3.getTagId()){
          ...
          FireError fireError3 = pnrd3.fire(2);

          switch (fireError3){
              case FireError::NO_ERROR :
                //Atualizando a tag
                if(pnrd3.saveData() == WriteError::NO_ERROR){
                      serial_com(tagId3,readerId,3,"NO_ERROR");
                      return;
                }else{
                      serial_com(tagId3,readerId,3,"ERROR_TAG_UPDATE");
                      return;
                }

              case FireError::PRODUCE_EXCEPTION :
                  serial_com(tagId3,readerId,3,"PRODUCE_EXCEPTION");
              return;

              case FireError::CONDITIONS_ARE_NOT_APPLIED :
                  serial_com(tagId3,readerId,3,"CONDITIONS_ARE_NOT_APPLIED");
              break;

              case FireError::NOT_KNOWN:
                serial_com(tagId3,readerId,3,"NOT_KNOWN");
              break; 
          }  
      }
  }           
